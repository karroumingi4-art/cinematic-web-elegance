...
// CASI GITHUB - metti qui le tue immagini con 2 foto per caso
// le foto devono stare in /public - es. /public/oro1.jpg
const CASI_DA_GITHUB: Caso[] = [
  {
    id: "24-07",
    titolo: "Terza maglia 24/25: ORO PURO o NERO OPACO?",
    imgs: ["/hero-tunnel-oro.jpg", "/hero-tunnel-oro.jpg"],
  ...
  },
  {
    ...
    imgs: ["/hero-tunnel-oro.jpg", "/hero-tunnel-oro.jpg"],
    opzioni: ["ROSSI #10", "BIANCHI #4"],
    voti: { "ROSSI #10": 3120, "BIANCHI #4": 1455 },
...
export function TribunaleTifoso() {
  ...
  const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const nuovo: Caso = { id: Date.now().toString(), titolo: form.titolo, imgs: previewImgs, opzioni, voti: generaVoti(opzioni), createdAt: Date.now() };
  ...
  return (
    <section id="tribunale" className="bg-[#080600] py-24 border-t border-white/10 scroll-mt-24">
      <div className="mx-auto max-w-7xl px-5">
            ...
            return (
              <div key={c.id} className="rounded- border border-white/10 bg-white/[0.03] p-7">
                {c.imgs && c.imgs.length > 0 && (
                  <div className={c.imgs.length > 1? "grid gap-2 mb-6 grid-cols-2" : "grid gap-2 mb-6 grid-cols-1"}>
                    {c.imgs.map((src, i) => (<img key={i} src={src} className="h-48 w-full object-cover rounded-xl" alt="" />))}
                  </div>
                )}
                ...
                <div className="mt-6 space-y-3">
                  {c.opzioni.map((op) => {
                    ...
                    return (<div key={op}><div className="flex justify-between text-[0.7rem] uppercase tracking-widest text-white/60 mb-1.5"><span>{op}</span><span>{c.voti[op].toLocaleString()} - {perc}%</span></div><div className="h-2 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-[#d6b45a]" style={{ width: perc + "%" }} /></div></div>);
                  })}
                </div>
                <div className="mt-6 grid grid-cols-2 gap-3">
                  {c.opzioni.map((op) => (<button key={op} disabled={votedIds.includes(c.id)} onClick={() => handleVote(c.id, op)} className={votedIds.includes(c.id)? "rounded-full py-3 text-[0.75rem] font-bold uppercase bg-white/10 text-white/30" : "rounded-full py-3 text-[0.75rem] font-bold uppercase bg-white text-black hover:bg-[#d6b45a]"}>{votedIds.includes(c.id)? "Votato" : "Vota " + op}</button>))}
                </div>
        ...
        <button onClick={() => setShowArchivio(!showArchivio)} className="mt-12 mx-auto flex items-center gap-2 rounded-full border border-white/20 px-6 py-3 text-[0.75rem] uppercase tracking-widest text-white hover:bg-white hover:text-black"><Archive className="size-4" />{showArchivio? "Chiudi Archivio" : "Apri Archivio (" + archivio.length + " casi)"}</button>
        {showArchivio && (<div className="mt-8 grid md:grid-cols-3 gap-4">{archivio.map((c) => {const vincitore = getVincitore(c.voti); return (<div key={c.id} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">{c.imgs && c.imgs.length > 0 && (<div className="grid grid-cols-2 gap-2 mb-3">{c.imgs.map((s, i) => (<img key={i} src={s} className="h-20 w-full object-cover rounded-lg" alt="" />))}</div>)}<h4 className="text-white font-bold text-sm">{c.titolo}</h4><p className="mt-2 text-[0.7rem] text-[#d6b45a] flex items-center gap-1"><Trophy className="size-3" /> VINCITORE: {vincitore}</p></div>);})}</div>)}
        <form onSubmit={handleAdd} encType="multipart/form-data" className="mt-20 rounded- border border-[#d6b45a]/20 p-8 bg-[#d6b45a]/5">
          <h3 className="text-white text-2xl font-bold">Proponi un nuovo caso con 2 immagini</h3>
          ...
          <input name="opzioni" value={form.opzioni} onChange={(e) => setForm({...form, opzioni: e.target.value})} required placeholder="Opzioni: ORO, NERO" className="mt-4 w-full rounded-full bg-black/40 border border-white/10 px-6 py-3 text-white text-sm" />
          <div className="mt-4"><label className="text-[0.7rem] uppercase tracking-widest text-white/50">Carica 2 immagini</label><input name="immagini" type="file" accept="image/*" multiple required onChange={handleFiles} className="mt-2 w-full text-sm text-white/60 file:mr-4 file:rounded-full file:border-0 file:bg-white file:px-4 file:py-2 file:text-black" /></div>
          {previewImgs.length > 0 && (<div className="mt-4 grid grid-cols-2 gap-3">{previewImgs.map((src, i) => (<img key={i} src={src} className="h-32 w-full object-cover rounded-xl border border-white/20" alt="preview" />))}</div>)}
          <button type="submit" className="mt-6 w-full rounded-full bg-[#d6b45a] py-4 font-bold uppercase text-black">Crea Caso Live con 2 Foto</button>
        </form>
      </div>
    </section>
  );
