...
const CASI_DA_GITHUB: Caso[] = tribunaleData.map((c: any) => ({
  id: c.id,
  titolo: c.titolo,
  opzioni: c.opzioni,
  voti: c.voti || generaVoti(c.opzioni),
  imgs: c.imgs?.map((k: string) => tribunaleImageMap[k] || k),
...
export function TribunaleTifoso() {
  ...
  const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if(opzioni.length<2) return alert("Min 2 opzioni separate da virgola");
              ...
              <div key={c.id} className="rounded- border border-white/10 bg-white/[0.03] p-7">
                ...
                <div className="mt-6 space-y-3">
                  {c.opzioni.map(op=>{
                    ...
                    return <div key={op}><div className="flex justify-between text-[0.7rem] uppercase tracking-widest text-white/60 mb-1.5"><span>{op}</span><span>{c.voti[op].toLocaleString()} · {perc}%</span></div><div className="h-2 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-[#d6b45a] transition-all duration-700" style={{width:`${perc}%`}}/></div></div>
                  })}
                </div>
                ...
                <div key={c.id} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                  {c.imgs && c.imgs.length>0 && (
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      {c.imgs.map((s,i)=><img key={i} src={s} className="h-20 w-full object-cover rounded-lg" alt=""/>)}
                    </div>
                  )}
                  <h4 className="text-white font-bold text-sm">{c.titolo}</h4>
                  <p className="mt-2 text-[0.7rem] text-[#d6b45a] flex items-center gap-1"><Trophy className="size-3"/> VINCITORE: {vincitore}</p>
                  <p className="text-[0.65rem] text-white/30 mt-2">{getTotale(c.voti).toLocaleString()} voti · esito definitivo</p>
                </div>
              )
            })}
          </div>
        ...
        <form onSubmit={handleAdd} encType="multipart/form-data" className="mt-20 rounded- border border-[#d6b45a]/20 p-8 bg-[#d6b45a]/5">
          ...
          {previewImgs.length>0 && (
            <div className="mt-4 grid grid-cols-2 gap-3">
              {previewImgs.map((src,i)=><img key={i} src={src} className="h-32 w-full object-cover rounded-xl border border-white/20" alt="preview"/>)}
            </div>
          )}
