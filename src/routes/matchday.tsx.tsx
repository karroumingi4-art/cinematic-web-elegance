    ...
    <div className="min-h-screen bg-[#080808] text-white pt-20">
      <div className="mx-auto max-w-[1600px] px-4 py-6 grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-6 bg-[#111] border border-white/10 rounded-2xl p-6">
          ...
          <div className="relative mx-auto w-full max-w-[750px] h-[720px] bg-[#0f0f0f] rounded-[24px] border border-white/10 p-2">
                  ...
                  return (
                    <div key={fila} className="grid grid-cols-3 gap-1.5">
                      {(["ALTO","CENTRALE","BASSO"] as const).map(sez => {
                        ...
                        return <button key={sez} onClick={()=>selectSector(trib, sez, fila)} className={`h-8 rounded-lg text-[10px] font-black border transition ${isSel?"bg-white text-black border-white scale-105":"bg-[#1e1e1e] text-white border-white/10 hover:border-white/30 hover:bg-[#2a2a2a]"}`}>{sez[0]}-F{fila} {price}€</button>
                      })}
                    </div>
                  )
                })}
            ...
            <div className="absolute left-2 top-[90px] bottom-[90px] w-[80px]">
              <div className="h-full flex flex-col gap-1.5">
                <div className="text-center text-[10px] font-black text-green-400 tracking-widest -rotate-90 origin-center mt-10 mb-10">VERDE</div>
                {Array.from({length:7}).map((_, idx) => {
                  const fila = 7-idx;
                  return (
                    <div key={fila} className="grid grid-rows-3 gap-1 flex-1">
                      {(["ALTO","CENTRALE","BASSO"] as const).map(sez => {
                        ...
                        const price = calcPrice(trib.base, fila, sez==="CENTRALE");
                        const isSel = selected?.tribuna===trib.nome && selected?.sez===sez && selected?.fila===fila;
                        return <button key={sez} onClick={()=>selectSector(trib, sez, fila)} className={`rounded-lg text-[9px] font-black border ${isSel?"bg-white text-black":"bg-[#1e1e1e] text-white border-white/10 hover:bg-[#2a2a2a]"}`}>{price}€</button>
                      })}
                    </div>
                  )
                })}
            ...
            <div className="absolute right-2 top-[90px] bottom-[90px] w-[80px]">
              <div className="h-full flex flex-col gap-1.5">
                <div className="text-center text-[10px] font-black text-orange-400 tracking-widest rotate-90 origin-center mt-10 mb-10">CURVA</div>
                {Array.from({length:7}).map((_, idx) => {
                  const fila = idx+1;
                  return (
                    <div key={fila} className="grid grid-rows-3 gap-1 flex-1">
          ...
          <div className="text-center text-[9px] opacity-30 mt-4">F1 = più vicino al campo (più caro) | F7 = più lontano (più economico) | Centrale costa 25% in più</div>
        </div>
          ...
          <div className="mt-4 text-xs space-y-2">
            ...
            {selected ? (
              <>
                ...
                <p className="text-[9px] opacity-40 text-center mt-2">Formspree attivo: xgawolng</p>
              </>
          ...
          <div className="w-full max-w-[900px]">
            <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl overflow-hidden relative flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-[220px] bg-[#151515] border-t md:border-t-0 md:border-l border-dashed border-white/20 p-6 flex flex-col items-center justify-center">
                ...
                <div className="text-[8px] opacity-40 mt-1 text-center">NON CEDIBILE - 1 INGRESSO</div>
              </div>
            </div>
            ...
            <p className="text-[10px] opacity-40 text-center mt-3">Inviato via Formspree a {form.email} - ID: xgawolng</p>
          </div>
        </div>
      )}
    </div>
