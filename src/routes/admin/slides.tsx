...
function AdminSlides() {
  ...
  const updStyle = (key:string, field:string, v:any)=>{ const ns=[...slides]; if(!ns[sel].styles) ns[sel].styles={}; if(!ns[sel].styles[key]) ns[sel].styles[key]={}; ns[sel].styles[key][field]=v; setSlides(ns); };
  ...
  const saveOnline = async ()=>{
    ...
    try{
      ...
      const putRes=await fetch(`https://api.github.com/repos/${repo}/contents/${path}`,{method:"PUT",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({message:"editor testo",content,sha:fd.sha,branch:"main"})});
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex">
      ...
      <div className="w- bg-[#111] border-r border-white/10 p-3 h- overflow-auto space-y-4">
        {/* SFONDO */}
        <div className="bg-black/50 p-3 rounded-xl border border-white/10"><div className="text- font-black opacity-50 mb-2">SFONDO</div><input type="color" value={s.bgColor||"#1a0a0f"} onChange={e=>upd("bgColor",e.target.value)} className="w-full h-8 rounded bg-black" /><input value={s.bgImage||""} onChange={e=>upd("bgImage",e.target.value)} placeholder="/sfondo.jpg" className="w-full mt-2 bg-black border border-white/10 rounded p-2 text-" /></div>

        {/* SOTTOTITOLO */}
        <div className="bg-black/50 p-3 rounded-xl border border-white/10">
          <div className="text- font-black text-[#7DD3E0] mb-2">SOTTOTITOLO</div>
          <input value={s.sottotitolo} onChange={e=>upd("sottotitolo",e.target.value)} className="w-full bg-black border border-white/10 rounded-lg p-2 text-xs mb-2" />
          <div className="grid grid-cols-2 gap-2"><select value={s.styles?.sottotitolo?.font||"Inter"} onChange={e=>updStyle("sottotitolo","font",e.target.value)} className="bg-black border border-white/10 rounded p-1 text-">{fonts.map(f=><option key={f}>{f}</option>)}</select><input type="range" min="8" max="30" value={s.styles?.sottotitolo?.size||10} onChange={e=>updStyle("sottotitolo","size",parseInt(e.target.value))} /></div>
          <div className="text- opacity-50">size: {s.styles?.sottotitolo?.size||10}px</div>
        </div>

        {/* TITOLO */}
        <div className="bg-black/50 p-3 rounded-xl border border-white/10">
          <div className="text- font-black text-white mb-2">TITOLO</div>
          <textarea value={s.titolo} onChange={e=>upd("titolo",e.target.value)} rows={2} className="w-full bg-black border border-white/10 rounded-lg p-2 text-sm font-black mb-2" />
          <div className="grid grid-cols-2 gap-2"><select value={s.styles?.titolo?.font||"Anton"} onChange={e=>updStyle("titolo","font",e.target.value)} className="bg-black border border-white/10 rounded p-1 text-">{fonts.map(f=><option key={f}>{f}</option>)}</select><input type="range" min="20" max="90" value={s.styles?.titolo?.size||36} onChange={e=>updStyle("titolo","size",parseInt(e.target.value))} /></div>
          <div className="flex gap-2 mt-2"><input type="color" value={s.textColor||"#ffffff"} onChange={e=>upd("textColor",e.target.value)} className="h-6 w-full rounded" /><span className="text- opacity-50">size {s.styles?.titolo?.size||36}px</span></div>
        </div>

        {/* TESTO */}
        <div className="bg-black/50 p-3 rounded-xl border border-white/10">
          <div className="text- font-black text-white/60 mb-2">TESTO</div>
          <textarea value={s.testo} onChange={e=>upd("testo",e.target.value)} rows={3} className="w-full bg-black border border-white/10 rounded-lg p-2 text-xs mb-2" />
          <div className="grid grid-cols-2 gap-2"><select value={s.styles?.testo?.font||"Inter"} onChange={e=>updStyle("testo","font",e.target.value)} className="bg-black border border-white/10 rounded p-1 text-">{fonts.map(f=><option key={f}>{f}</option>)}</select><input type="range" min="10" max="28" value={s.styles?.testo?.size||12} onChange={e=>updStyle("testo","size",parseInt(e.target.value))} /></div>
          <div className="text- opacity-50">size {s.styles?.testo?.size||12}px</div>
        </div>
