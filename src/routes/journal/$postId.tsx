...
function JournalPost() {
  ...
  const nextPost = posts[(index + 1) % posts.length]; // prossimo, se è l'ultimo torna al primo
  const prevPost = posts[(index - 1 + posts.length) % posts.length];

  return (
    <div className="min-h-screen bg-ink text-white">
      <div className="mx-auto max-w-3xl px-5 py-12 sm:py-24">
        <Link to="/" hash="journal" className="inline-flex items-center gap-2 text-[0.7rem] tracking-[0.2em] uppercase text-primary hover:text-white transition">
        ...
        <h1 className="display mt-6 text-4xl sm:text-6xl leading-[0.9] tracking-tight">{post.title}</h1>
        ...
        <div className="mt-20 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-white/10 pt-10">
          ...
          <Link to="/journal/$postId" params={{ postId: prevPost.id }} className="group rounded-2xl bg-white/[0.04] border border-white/10 p-6 hover:bg-white/[0.08] transition">
            ...
            <div className="mt-2 font-bold group-hover:text-[#95BFE5] transition">{prevPost.title}</div>
          </Link>
          ...
          <Link to="/journal/$postId" params={{ postId: nextPost.id }} className="group rounded-2xl bg-[#95BFE5] text-black p-6 hover:bg-white transition">
            ...
            <div className="mt-2 font-bold flex items-center gap-2">
              {nextPost.title} <ArrowUpRight className="size-4 shrink-0 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
            </div>
          </Link>
