...
function MagliaVideo() {
  return (
    <div className="relative w-full aspect-[16/9] bg-gradient-to-br from-[#1a0a12] via-[#2B0A14] to-[#5C0A2D] rounded-2xl overflow-hidden border border-white/10 mt-12">
      <div className="absolute -top-20 -right-20 w-72 h-72 bg-[#B8E6F0]/20 blur- rounded-full" />
      <div className="absolute -bottom-20 -left-20 w-72 h-72 bg-[#FFD700]/10 blur- rounded-full" />
      <div className="absolute inset-0 flex items-center justify-center animate-[float_3s_ease-in-out_infinite]">
        <img src={imageMap["maglia-trasparente"]} className="w-[58%] h-auto drop-shadow-[0_20px_40px_rgba(0,0,0,0.6)] object-contain" alt="Maglia" />
      </div>
      ...
      <div className="absolute top-5 left-5 flex gap-2">
        <div className="bg-white text-black text- font-black px-2.5 py-1 rounded-full">2026-27</div>
        <div className="bg-[#B8E6F0] text-black text- font-black px-2.5 py-1 rounded-full">BORDEAUX × CELESTE</div>
      </div>
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent">
        <div className="flex gap-2 mb-3">
          <div className="h- w-10 bg-[#5C0A2D]" />
          <div className="h- w-10 bg-[#B8E6F0]" />
          <div className="h- w-10 bg-[#FFD700]" />
        </div>
        <h3 className="text-white font-black text- md:text- leading-[0.9]">LA MAGLIA<br/>PER IL RITORNO</h3>
        <p className="text-white/50 text- tracking-[0.2em] mt-2">CON LA STELLA DELLO SCUDETTO 2024 • TRENTO 6 SETTEMBRE</p>
      </div>
      <style>{`@keyframes float{0%,100%{transform:translateY(0px) rotateY(-5deg)}50%{transform:translateY(-12px) rotateY(5deg)}}@keyframes ping{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.2);opacity:0.8}}`}</style>
    </div>
  );
}
