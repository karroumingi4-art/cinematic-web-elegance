...
const TRIBUNE_CONFIG = [
  { id: "rossa", nome: "ROSSA", base: 320, color: "bg-red-500" },
  { id: "blu", nome: "BLU", base: 240, color: "bg-blue-500" },
  { id: "verde", nome: "VERDE", base: 150, color: "bg-green-500" },
  { id: "arancio", nome: "CURVA", base: 80, color: "bg-orange-500" },
] as const;
...
function MatchdayPage() {
  ...
  async function handlePay() {
    ...
    try {
      await fetch(FORMSPREE_ENDPOINT, {
        ...
        body: JSON.stringify({
          partita: `${selectedMatch.home} vs ${selectedMatch.away} - ${selectedMatch.date} ${selectedMatch.time} - ${selectedMatch.competition}`,
  ...
  function downloadTicket() {
    ...
    let x = 60;
    infos.forEach(([k,v]) => { ctx.fillStyle = "#666"; ctx.font = "10px monospace"; ctx.fillText(k, x, 180); ctx.fillStyle = "white"; ctx.font = "bold 20px sans-serif"; ctx.fillText(v, x, 210); x += 150; });
    ...
    ctx.fillStyle = "black"; for(let i=0;i<100;i++){ if(Math.random()>0.5) ctx.fillRect(910 + (i%10)*18, 90 + Math.floor(i/10)*18, 14,14); }
      ...
      <div className="mx-auto max-w- px-4 py-6 grid grid-cols-12 gap-6">
        <div className="col-span-12 lg:col-span-3 bg-[#111] border border-white/10 rounded-2xl p-3 h- overflow-y-auto">
          ...
          {MATCHES.map(m => (
            <button key={m.id} onClick={()=>setSelectedMatch(m)} className={`w-full text-left p-3 rounded-xl mb-2 border transition ${selectedMatch.id===m.id?"bg-white text-black border-white":"bg-black border-white/10 hover:border-white/20"}`}>
              ...
              <div className="text- opacity-50">{m.venue}</div>
            </button>
          ))}
        </div>
        ...
        <div className="col-span-12 lg:col-span-6 bg-[#111] border border-white/10 rounded-2xl p-6">
          <div className="text-center mb-6">
            <div className="text- tracking-widest opacity-40">SAN SIRO - 84 SETTORI - ANELLO COMPLETO</div>
            <div className="font-black text-lg mt-1">{selectedMatch.home} vs {selectedMatch.away}</div>
            <div className="text- opacity-50">{selectedMatch.date} {selectedMatch.time}</div>
          </div>

          <div className="relative mx-auto w-full max-w- h- bg-[#0f0f0f] rounded- border border-white/10 p-2">
            <div className="absolute inset- bg-[#1a6b1a] rounded-xl border border-white/10 flex items-center justify-center">
            ...
            <div className="absolute top-2 left- right-">
              <div className="text-center text- font-black text-red-400 tracking-widest mb-2">TRIBUNA ROSSA</div>
                  ...
                  return (
                    <div key={fila} className="grid grid-cols-3 gap-1.5">
                      {(["ALTO","CENTRALE","BASSO"] as const).map(sez => {
                        ...
                        return <button key={sez} onClick={()=>selectSector(trib, sez, fila)} className={`h-8 rounded-lg text- font-black border transition ${isSel?"bg-white text-black border-white scale-105":"bg-[#1e1e1e] text-white border-white/10 hover:border-white/30 hover:bg-[#2a2a2a]"}`}>{sez[0]}-F{fila} {price}€</button>
                      })}
                    </div>
                  )
                })}
            ...
            <div className="absolute bottom-2 left- right-">
              <div className="space-y-1">
                {Array.from({length:7}).map((_, idx) => {
                  const fila = idx+1;
                  return (
              ...
              <div className="text-center text- font-black text-blue-400 tracking-widest mt-2">TRIBUNA BLU</div>
            </div>

            <div className="absolute left-2 top- bottom- w-">
              <div className="h-full flex flex-col gap-1.5">
                <div className="text-center text- font-black text-green-400 tracking-widest -rotate-90 origin-center mt-10 mb-10">VERDE</div>
                {Array.from({length:7}).map((_, idx) => {
                  const fila = 7-idx;
                  return (
                    <div key={fila} className="grid grid-rows-3 gap-1 flex-1">
                      {(["ALTO","CENTRALE","BASSO"] as const).map(sez => {
                        ...
                        const price = calcPrice(trib.base, fila, sez==="CENTRALE");
                        const isSel = selected?.tribuna===trib.nome && selected?.sez===sez && selected?.fila===fila;
                        return <button key={sez} onClick={()=>selectSector(trib, sez, fila)} className={`rounded-lg text- font-black border ${isSel?"bg-white text-black":"bg-[#1e1e1e] text-white border-white/10 hover:bg-[#2a2a2a]"}`}>{price}€</button>
                      })}
                    </div>
                  )
                })}
            ...
            <div className="absolute right-2 top- bottom- w-">
              <div className="h-full flex flex-col gap-1.5">
                <div className="text-center text- font-black text-orange-400 tracking-widest rotate-90 origin-center mt-10 mb-10">CURVA</div>
                {Array.from({length:7}).map((_, idx) => {
                  const fila = idx+1;
                  return (
                    <div key={fila} className="grid grid-rows-3 gap-1 flex-1">
          ...
          <div className="text-center text- opacity-30 mt-4">F1 = più vicino al campo (più caro) | F7 = più lontano (più economico) | Centrale costa 25% in più</div>
        </div>
          ...
          <div className="mt-4 text-xs space-y-2">
            ...
            {selected? (
              <>
                ...
                <button onClick={()=>setShowPay(true)} className="mt-6 w-full bg-white text-black rounded-full py-4 font-black text-xs tracking-widest hover:bg-zinc-200">PAGA {selected.price * qty}€</button>
                <p className="text- opacity-40 text-center mt-2">Formspree attivo: xgawolng</p>
              </>
            ) : <div className="opacity-40 text-xs py-10 text-center">Seleziona un settore nello stadio ad anello</div>}
          </div>
        </div>
      </div>
      ...
      {showPay && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur flex items-center justify-center z-50 p-4">
          <div className="bg-[#111] border border-white/10 rounded-2xl p-6 w-full max-w-md">
            ...
            <p className="text-xs opacity-60 mt-1">Formspree: https://formspree.io/f/xgawolng - ATTIVO</p>
            <div className="mt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input placeholder="MM/AA" defaultValue="12/28" className="bg-black border border-white/10 rounded-xl px-4 py-3 text-sm" />
                <input placeholder="CVC" defaultValue="123" className="bg-black border border-white/10 rounded-xl px-4 py-3 text-sm" />
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-">
            <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl overflow-hidden relative flex flex-col md:flex-row">
              <div className="w-10 bg-[#C8102E] hidden md:flex items-center justify-center"><span className="text-white font-black text- tracking-widest -rotate-90 whitespace-nowrap">GASTON VILLA PARK - SAN SIRO</span></div>
